/*global Ext */
//Ext.operon.tutorials.Tutorial=Ext.extend(Ext.util.Observable,
Ext.operon.tutorials.Tutorial=Ext.extend(Ext.Panel,
{
	// panel config
	layout: 'table',
	...
	constructor:function(data)
	{
		...
		Ext.operon.tutorials.Tutorial.superclass.constructor.call(this,config);
		...
		{
			this.pages.push(new Ext.operon.tutorials.Page(this,data.pages[index]));
		}
		if (data.questions)
		{
			for (index=0;index<data.questions.length;index++)
			{
				this.questions.push(new Ext.operon.tutorials.Question(this,data.questions[index]));
			}
		}
		if (data.images)
		{
			...
			{
				this.images.push(new Ext.operon.tutorials.Image(this,data.images[index]));
			}
		}
		if (data.links)
		{
			...
			{
				this.links.push(new Ext.operon.tutorials.Link(this,data.links[index]));
			}
		}
		if (data.flashs)
		{
			...
			{
				this.flashs.push(new Ext.operon.tutorials.Flash(this,data.flashs[index]));
			}
		}
		this.toc=new Ext.operon.tutorials.Toc(this);
		this.score=new Ext.operon.tutorials.Score(this);
		this.indexResources();
		...
		Ext.DomHelper.overwrite('tutorial_title',this.number+'. '+this.tutorial_title);
		//Ext.DomHelper.overwrite('tutorial_date',new Date());
		this.toc.update();
